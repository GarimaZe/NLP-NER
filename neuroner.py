...
class NeuroNER(object):
    ...
    def _load_parameters(self, parameters_filepath, arguments={}, verbose=True):
        ...
        '''
        parameters = {'pretrained_model_folder':'../trained_models/conll_2003_zh',
                      'dataset_text_folder':'../data/conll2003/zh',
    ...
    def _get_valid_dataset_filepaths(self, parameters,spacy_nlp, dataset_types=['train', 'valid', 'test', 'deploy']):
        dataset_filepaths = {}
            ...
            # Conll file does not exist
                ...
                if os.path.exists(dataset_brat_folders[dataset_type]) and len(glob.glob(os.path.join(dataset_brat_folders[dataset_type], '*.txt'))) > 0:
                    ...
                    if os.path.exists(dataset_filepath_for_tokenizer):
                        ...
                        brat_to_conll.brat_to_conll(dataset_brat_folders[dataset_type], dataset_filepath_for_tokenizer, parameters['tokenizer'], parameters['spacylanguage'],spacy_nlp)
    ...
    def __init__(self,
        ...
        dataset_filepaths, dataset_brat_folders = self._get_valid_dataset_filepaths(parameters,spacy_nlp)
        ...
        device_count={'CPU': 8, 'GPU': parameters['number_of_gpus']},
    ...
    def predict(self, text, spacy_nlp):
        ...
        start = time.time()
        output_String = brat_to_conll.brat_to_conll1(text, "spacy", "zh", spacy_nlp, self.prediction_count)
        #print(output_String.replace('\n','  '))
        # need to change function
        conll_output_String = utils_nlp.convert_conll_from_bio_to_bioes1(output_String)
        #print(conll_output_String.replace('\n', '  '))
        #dataset= Dataset()
        #end = time.time()
        #print('conll_output_String', end - start)
        #start= time.time()
        dataset2= ds.Dataset()
        labels, tokens, token_count, label_count, character_count= dataset2._parse_dataset1(conll_output_String);
        dataset_types=["deploy"]
        token_indices, label_indices, character_indices_padded, character_indices, token_lengths, characters, label_vector_indices=        dataset2._convert_to_indices1(dataset_types,tokens,labels,self.dataset.token_to_index,self.dataset.character_to_index,self.dataset.label_to_index,self.dataset.index_to_label,self.dataset.UNK_TOKEN_INDEX,self.dataset.index_to_character,self.dataset.PADDING_CHARACTER_INDEX,self.dataset.verbose);
        dataset2.alphabet_size = self.dataset.alphabet_size
        dataset2.character_indices = character_indices
        dataset2.character_indices_padded =character_indices_padded
        dataset2.character_to_index = self.dataset.character_to_index
        dataset2.characters = characters
        dataset2.debug = self.dataset.debug
        dataset2.index_to_character = self.dataset.index_to_character
        dataset2.index_to_label = self.dataset.index_to_label
        dataset2.index_to_token =self.dataset. index_to_token
        dataset2.infrequent_token_indices = self.dataset.infrequent_token_indices
        dataset2.label_indices = label_indices
        dataset2.label_to_index =self.dataset.label_to_index
        dataset2.label_vector_indices = label_vector_indices
        dataset2.labels = labels
        dataset2.name = self.dataset.name
        dataset2.number_of_classes = self.dataset.number_of_classes

        dataset2.PADDING_CHARACTER_INDEX = self.dataset.PADDING_CHARACTER_INDEX
        dataset2.token_indices = token_indices
        dataset2.token_lengths = token_lengths
        dataset2.token_to_index = self.dataset.token_to_index
        dataset2.tokens = tokens
        dataset2.tokens_mapped_to_unk = self.dataset.tokens_mapped_to_unk
        dataset2.unique_label_indices_of_interest = self.dataset.unique_label_indices_of_interest
        dataset2.unique_labels = self.dataset.unique_labels
        dataset2.unique_labels_of_interest = self.dataset.unique_labels_of_interest
        dataset2.UNK = self.dataset.UNK
        dataset2.UNK_TOKEN_INDEX = self.dataset.UNK_TOKEN_INDEX
        dataset2.verbose = self.dataset.verbose
        dataset2.vocabulary_size = self.dataset.vocabulary_size
        #end = time.time()
        #print('DataSet Processing', end - start)
        #print(labels, tokens, token_count, label_count, character_count)
        #print(self.dataset.tokens)

        output_filepaths = {}

        
        prediction_output = train.prediction_step1(self.sess, dataset2, dataset_type, self.model,self.transition_params_trained, self.parameters,  conll_output_String)
        _, _, output_filepaths[dataset_type] = prediction_output

        annotation_filepath = conll_to_brat.output_brat1(text, output_filepaths["deploy"], output_String,overwrite=True)
        #print(annotation_filepath)
        text2, entities = brat_to_conll.get_entities_from_brat1(text, annotation_filepath, verbose=True)
        #print(entities)
        end = time.time()
