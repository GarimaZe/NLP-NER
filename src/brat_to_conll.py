...
def brat_to_conll(input_folder, output_filepath, tokenizer, language,spacy_nlp):
    '''
...
def get_entities_from_brat1(text, annotationString, verbose=False):
    # load text
    '''with codecs.open(text_filepath, 'r', 'UTF-8') as f:
        text =f.read()'''
    #if verbose: print("\ntext:\n{0}\n".format(text))
    entities = []
    if annotationString is None:
        return text,entities
    # parse annotation file
    if annotationString == "":
        return text,entities
    if annotationString is str:
        lines=annotationString.split('\n')
    else:
        lines = annotationString
    for line in lines:
        anno = line.split()
        id_anno = anno[0]
        # parse entity
...
def brat_to_conll1(text, tokenizer, language, spacy_nlp, prediction_count):
    '''
    Assumes '.txt' and '.ann' files are in the input_folder.
    Checks for the compatibility between .txt and .ann at the same time.
    '''
    ...
    # text_filepaths = sorted(glob.glob(os.path.join(input_folder, '*.txt')))
    # output_file = codecs.open(output_filepath, 'w', 'utf-8')
    text, entities = get_entities_from_brat1(text, "")
    entities = sorted(entities, key=lambda entity: entity["start"])
    output_String = ""
    if tokenizer == 'spacy':
        sentences = get_sentences_and_tokens_from_spacy(text, spacy_nlp)
        ...
        sentences = get_sentences_and_tokens_from_stanford(text, core_nlp)
    base_filename = 'temp_{0}'.format(str(prediction_count).zfill(5))
    for sentence in sentences:
        inside = False
        previous_token_label = 'O'
        for token in sentence:
