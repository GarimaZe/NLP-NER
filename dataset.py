...
class Dataset(object):
    ...
    def _parse_dataset1(self, dataset_filepath):
        token_count = collections.defaultdict(lambda: 0)
        label_count = collections.defaultdict(lambda: 0)
        character_count = collections.defaultdict(lambda: 0)
    ...
    def _convert_to_indices1(self, dataset_types,tokens,labels,token_to_index,character_to_index,label_to_index,index_to_label,UNK_TOKEN_INDEX,index_to_character,PADDING_CHARACTER_INDEX,verbose):
        ...
        for dataset_type in dataset_types:
            ...
            for token_sequence in tokens:
                token_indices[dataset_type].append(
                    [token_to_index.get(token, UNK_TOKEN_INDEX) for token in token_sequence])
                characters[dataset_type].append([list(token) for token in token_sequence])
                character_indices[dataset_type].append([[character_to_index.get(character, random.randint(1, max(
                    index_to_character.keys()))) for character in token] for token in token_sequence])
                token_lengths[dataset_type].append([len(token) for token in token_sequence])
                longest_token_length_in_sequence = max(token_lengths[dataset_type][-1])
                character_indices_padded[dataset_type].append(
                    [utils.pad_list(temp_token_indices, longest_token_length_in_sequence, PADDING_CHARACTER_INDEX)
    ...
    def update_dataset1(self, dataset_filepaths, dataset_types):
        '''
        dataset_filepaths : dictionary with keys 'train', 'valid', 'test', 'deploy'
        Overwrites the data of type specified in dataset_types using the existing token_to_index, character_to_index, and label_to_index mappings.
        '''
        for dataset_type in dataset_types:
            self.labels[dataset_type], self.tokens[dataset_type], _, _, _ = self._parse_dataset1(dataset_filepaths)
    ...
    def update_dataset2(self, dataset_filepaths, dataset_types):
        '''
        dataset_filepaths : dictionary with keys 'train', 'valid', 'test', 'deploy'
        Overwrites the data of type specified in dataset_types using the existing token_to_index, character_to_index, and label_to_index mappings.
        '''
