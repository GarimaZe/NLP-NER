...
def output_conll_lines_with_bioes1(split_lines, labels):
    '''
    Helper function for convert_conll_from_bio_to_bioes
    '''
    output_conll_String=""
    ...
    assert(len(new_labels) == len(split_lines))
    for split_line, new_label in zip(split_lines, new_labels):
        output_conll_String=output_conll_String+str(' '.join(split_line + [new_label]) + '\n')
    del labels[:]
    del split_lines[:]
    return output_conll_String

def convert_conll_from_bio_to_bioes1(input_conll_String):
    output_conll_String = ""
    labels = []
    split_lines = []
    lines = input_conll_String.split('\n')
    for line in lines:
        split_line = line.strip().split(' ')
        # New sentence

        if len(split_line) == 0 or len(split_line[0]) == 0 or '-DOCSTART-' in split_line[0]:
            str1 = output_conll_lines_with_bioes1(split_lines, labels)
            # print(str1,'\n')
            if str1 is not None:
                output_conll_String = output_conll_String + str1
            if line is not None:
                if line == '':
                    output_conll_String = output_conll_String + str('\n')
                else:
                    output_conll_String = output_conll_String + str(line)
            continue
        label = split_line[-1]
        labels.append(label)
        split_lines.append(split_line)
    str2 = output_conll_lines_with_bioes1(split_lines, labels)
    if str2 is not None:
        output_conll_String = output_conll_String + str2
